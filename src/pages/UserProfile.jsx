...
const UserProfile = () => {
  ...
  const changeAvatarHandler = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${import.meta.env.VITE_REACT_APP_BASE_URL}/users/change-avatar`, postData, { withCredentials: true, headers: { Authorization: `Bearer ${token}` } })
        ...
        <div className="profile__details">
          ...
          <form className="form profile__form" onSubmit={updateUserDetails}>
            ...
            <div className="input-field">
              <input type={showPassword ? 'password' : 'text'} placeholder='Current Password' value={currentPassword} onChange={e => setCurrentPassword(e.target.value)} />
              <span className='eye-icon' onClick={togglePassword}>
                {showPassword ? <IoMdEyeOff /> : <IoMdEye />}
              </span>
            </div>
            <div className="input-field">
              <input type={showPassword ? 'password' : 'text'} placeholder='New Password' value={newPassword} onChange={e => setNewPassword(e.target.value)} />
              <span className='eye-icon' onClick={togglePassword}>
                {showPassword ? <IoMdEyeOff /> : <IoMdEye />}
              </span>
            </div>
            <div className="input-field">
              <input type={showPassword ? 'password' : 'text'} placeholder='Confirm New Password' value={confirmNewPassword} onChange={e => setConfirmNewPassword(e.target.value)} />
              <span className='eye-icon' onClick={togglePassword}>
                {showPassword ? <IoMdEyeOff /> : <IoMdEye />}
              </span>
            </div>
